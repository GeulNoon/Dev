...
def wordList(temp):
    ...
    if len(test_word) < 1:  #index out of range 문제로 인해 default 값 추가, 해결되었는지 확인 필요
        test_word.append('유입')
        
    word_02 = test_sameword[:2]
    print(word_02)
    if len(word_02) == 0: #index out of range 문제로 인해 default 값 추가, 해결되었는지 확인 필요
        word_02 = ['인재','배'] 
    elif len(word_02) == 1:
        word_02.append('인재')
    ...
    if len(test_word) < 4:  #index out of range 문제로 인해 default 값 추가, 해결되었는지 확인 필요
        test_word.append('가치')
