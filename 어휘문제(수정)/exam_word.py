...
# 지문 & 출제기준1 (상, 중, 하), 출제기준2 (A,B,C) 등급 분리한 파일 불러오기
# 지문 -> 데이터베이스에서 가지고 와야함
text = ''
with open('C:/example/지문3.txt',encoding = 'cp949') as f:
    ...
    f.close()
#불용어제거 리스트
with open('stop_word.txt',encoding = 'cp949') as f:
    stop_word = f.readlines()
    f.close()

# 출제기준1 (A,B,C)
with open('WORD_A_01.txt',encoding = 'cp949') as f:
    WORD_A_01 = f.readlines()
    f.close()
with open('WORD_B_01.txt',encoding = 'cp949') as f:
    WORD_B_01 = f.readlines()
    f.close()
with open('WORD_C_01.txt',encoding = 'cp949') as f:
    WORD_C_01 = f.readlines()
    f.close() 
# 출제기준2(A,B,C)
with open('WORD_A_02.txt',encoding = 'cp949') as f:
    WORD_A_02 = f.readlines()
    f.close()
with open('WORD_B_02.txt',encoding = 'cp949') as f:
    WORD_B_02 = f.readlines()
    f.close()
with open('WORD_C_02.txt',encoding = 'cp949') as f:
...
word_03 = test_word[1]
#문제2번 출제 어휘
word_02 = random.choice(test_sameword)
